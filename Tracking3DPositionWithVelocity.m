...
while true
    ...
    if i == 1
        ...
        %T_dot = [last_T(X) - T(X), last_T(Y) - T(Y), last_T(Z) - T(Z)];
        T_dot = [(last_T(X) - T(X))/(trajectory(i,3)-trajectory(i-1,3)),...
            (last_T(Y) - T(Y))/(trajectory(i,3)-trajectory(i-1,3)),...
            (last_T(Z) - T(Z))/(trajectory(i,3)-trajectory(i-1,3))];
        
        %calculate the acceleration of the target, storing the previous
        last_T_accel = T_accel;
        T_accel = T_dot - last_Tdot;
        ...
        T_future = T + T_dot.*t1 + (T_accel.*t1^2)./2;
        ...
        T_aim = T + T_dot.*lead_time + (T_accel.*lead_time^2)./2;
